...
SUBID = 3;
SESSION = 0; % 0: Week 1, 1: Week 2

set.SUBID = SUBID;
set.SESSION = SESSION;
disp(['Subject ID: ' num2str(SUBID+SESSION)])

%% Options that can sometimes be fucked with for pilotting purposes

options.eyetracking = 1;
options.trigger = 1;

%% Options that shouldn't be fucked with
...
%% Monitor properties
mon.res = [1920 1080];%[1280 720]; %[1440 900];%[1920 1080];
mon.ref = 60; %144
mon.use = 0; %0

%% Experiment structure
n.blocks = 3; % 3 x 20 minute blocks

% 
% 
% % NFport = tcpip('10.50.74.64',1980, 'Networkrole','client');
% NFport = tcpip('10.50.74.64',1980, 'Networkrole','client');
% fopen(NFport)
% 
% 
% NFport =udp('10.50.74.64', 'RemotePort', 1980, 'LocalPort', 1980);
% fopen(NFport)
% 
% 
% fscanf(NFport)
...
% fread(NFport,1, 'int')
% fread(NFport)
% NFport.NumBytesAvailable
% NFport.BytesAvailable
% while 1
%     if NFport.BytesAvailable > 0
%         fread(NFport, NFport.BytesAvailable)
%     end
% end
% 
% fwrite(NFport, '7')
...
key.space = 32;
key.response = key.space;%[key.left key.right];
key.esc = 27;
...
% Sizing
stim.scale = (0.8/1920)*mon.res(1);

stim.width = (51/1920)*mon.res(1);
stim.height = (90/1080)*mon.res(2);
