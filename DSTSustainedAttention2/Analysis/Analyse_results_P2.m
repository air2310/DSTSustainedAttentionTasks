...
%% Set directories
direct.data = 'C:\Users\uqarento\OneDrive\DSTR\Pilots\Pilot2_lab\resultsDR\';
direct.results = 'C:\Users\uqarento\OneDrive\DSTR\Pilots\Pilot2_lab\resultsDR\';
direct.data_eeg = 'C:\Users\uqarento\OneDrive\DSTR\Pilots\Pilot2_lab\resultsDR\eeg\';
...
% load([direct.data 'BEHAVE_11-Nov-2021 12-00.mat'])
...
targetwise.resptime = DATA(targetonsetframes,Dkey.VBLTimestamp) ; % 0 = miss, 1 = hit- correct, 2 = hit - incorrect, 3 = false alarm. 
...
for ii_resp = 1:length( responses_idx )
    ...
    s.resptimeframe = 3;
    resptimeframe = [resp_time - s.resptimeframe, resp_time-0.15]; % valid timeframe
...
for ii_block = 1:settings.n.blocks
    ...
    stem(responsewise.time(responsewise.resptype==3 & idx_resp), responsewise.resptype(responsewise.resptype==3 & idx_resp), 'mo');
    stem(targetwise.resptime(targetwise.resptype==1 &  ii_targ), targetwise.resptype(targetwise.resptype==1 &  ii_targ), 'go');
    stem(targetwise.resptime(targetwise.resptype==2 &  ii_targ), targetwise.resptype(targetwise.resptype==2 &  ii_targ), 'yo');
    stem(targetwise.resptime(targetwise.resptype==0 &  ii_targ), targetwise.resptype(targetwise.resptype==0 &  ii_targ), 'co');
...
h = figure; 
plot(targetwise.resptime(targetwise.resptype==resp.hitCorrect), targetwise.RT(targetwise.resptype==resp.hitCorrect), '-x')
ylabel('RT (s)')
xlabel('Time (mins)')
title('RT over time')
...
filename.bids.EEG = 'sub-100_task-AttnNFMotion_day-1_phase-Test_eeg4';
