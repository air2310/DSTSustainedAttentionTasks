...
%% Set directories
direct.data = 'C:\Users\uqarento\OneDrive\DSTR\Pilots\Pilot2_lab\resultsAR\';
direct.results = 'C:\Users\uqarento\OneDrive\DSTR\Pilots\Pilot2_lab\resultsAR\';
direct.data_eeg = 'C:\Users\uqarento\OneDrive\DSTR\Pilots\Pilot2_lab\resultsAR\eeg\';
...
DATA(Didx(:, 2),Dkey.target_isonsetframe) = DATA(Didx(:, 1),Dkey.target_isonsetframe);
DATA(Didx(:, 3),Dkey.target_isonsetframe) = DATA(Didx(:, 1),Dkey.target_isonsetframe);
DATA(Didx(:, 4),Dkey.target_isonsetframe) = DATA(Didx(:, 1),Dkey.target_isonsetframe);


DATA(Didx(:, 2),Dkey.target_type) = DATA(Didx(:, 1),Dkey.target_type);
DATA(Didx(:, 3),Dkey.target_type) = DATA(Didx(:, 1),Dkey.target_type);
DATA(Didx(:, 4),Dkey.target_type) = DATA(Didx(:, 1),Dkey.target_type);
...
for ii_resp = 1:length( responses_idx )
    ...
    resptimeframe = [resp_time - 2.5, resp_time-0.15]; % valid timeframe
...
for ii_block = 1:settings.n.blocks
    ...
    for ii_split = 1:num_splits
        ...
        Twise.hits(ii_block, ii_split) = sum(targetwise.resptype(idxuse)==resp.hitCorrect);
...
for ii_measure = 1:length(twisefields)
    ...
    hold on;
    for block = 1:4
        plot(1:num_splits, Twise.(twisefields{ii_measure})(block,:)', '-o', 'color', colors(block,:), 'linewidth', 2)% 'facecolor', 'k')
    end
